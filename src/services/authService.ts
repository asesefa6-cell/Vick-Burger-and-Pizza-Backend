...
const resolveBusinessForUser = async (userId: string, businessId?: string | null) => {
...
export const loginUser = async (
  ...
  const resolvedBusinessId = await resolveBusinessForUser(user.id, user.businessId);
  ...
  let businesses: { id: string; businessName: string }[] | undefined = undefined;
  let business: { id: string; businessName: string } | null = null;

  if (roleName === 'Super Admin') {
    const links = await models.UserBusiness.findAll({ where: { userId: user.id } });
    businessIds = links.map((l) => l.businessId);
    if (businessIds.length > 0) {
      const rows = await models.Business.findAll({ where: { id: businessIds } });
      businesses = rows.map((b) => ({ id: b.id, businessName: b.businessName }));
    } else {
      businesses = [];
    }
...
export const getMe = async (
  userId: string
): Promise<{ id: string; name: string; email: string; role: string; businessId?: string | null; businessIds?: string[]; businesses?: { id: string; businessName: string }[]; business?: { id: string; businessName: string } | null; profileFileId?: string | null } | null> => {
  const user = await models.User.findByPk(userId, { include: [{ model: Role }] });
  if (!user) return null;
...
export const updateMe = async (
  ...
  await user.update({
    ...
    ...(updates.profileFileId !== undefined ? { profileFileId: updates.profileFileId } : {}),
  });
